...
The TimeTagger4 is available as a PCIe plugin board (variant “–PCIe”) or as a
...
.. figure:: _figures/xTDC4-TBT_xTDC4_PCIe.png
    ...
    :alt: PCIe- and TBT-variant of the TimeTagger4

    Overview of the PCIe (left) and the TBT (right) variant of the TimeTagger4.
...
The TimeTagger4 board can be installed in any PCIe-CEM slot with x1 or more lanes.
...
:numref:`fig schematic` shows a side view of the PCIe variant of the TimeTagger4.

.. figure:: _figures/xTDC4_Slotblende.*
    :name: fig bracket
    :alt: TimeTagger4 slot bracket
    :width: 50%

    Input connectors of the TimeTagger4 on the PCIe bracket.

.. figure:: _figures/xTDC4_schematic.*
    :name: fig schematic
    :alt: Schematic of the TimeTagger4 PCIe board.

    Schematic of the TimeTagger4 PCIe board. The connectors J12 and J25 are only
...
.. figure:: _figures/InputCircuit.*
    ...
    :alt: Input circuit of the TimeTagger4 TDC channels.
    :width: 35%
...
(see :c:member:`timetagger4_configuration.dc_offset`). The threshold can also be used
...
:c:member:`timetagger4_init_parameters.use_ext_clock` accordingly.

Only Gen 1 TimeTagger4 boards have inter-board connectors near the top edge of the 
